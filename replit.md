...
- **CMS Content Creation Dual-Path Handler**: The `/cms/content/create` endpoint handles both JavaScript FormData submissions (from the CMS interface) and traditional Flask WTForms submissions. JavaScript submissions are identified by the presence of the 'content_type' field (which the CMS JavaScript always sends), while traditional forms may not have this field. The endpoint correctly handles field names with underscores ('day_number', 'bot_id') as sent by the JavaScript. This ensures CMS greeting/welcome message creation works correctly while preserving backward compatibility with WTFforms-based workflows. The endpoint requires explicit bot_id for CMS submissions to prevent wrong-bot content creation.
- **Timezone-Based Content Scheduling**: Bots can be configured with specific timezones and scheduled delivery times for timezone-aware content delivery. Each bot maintains isolated timezone settings (e.g., Bot 1: America/New_York at 09:00, Bot 2: Asia/Jakarta at 19:00). The scheduler automatically detects bot configuration and switches between timezone-based scheduled delivery (once daily at specified local time) and interval-based delivery (every N minutes). Server-side validation prevents inconsistent configuration (delivery time without timezone). When both timezone and scheduled_delivery_time are set, the bot sends content once daily when the local time matches or exceeds the scheduled time. When either field is empty, the bot falls back to interval-based delivery using delivery_interval_minutes. This enables global reach with culturally appropriate delivery times for different regions.
- **Content Delivery Timing System**: Implements precise timing controls for optimal user engagement. When a user sends START, the system delivers the welcome message immediately, waits 10 seconds, then sends Day 1 content directly (bypassing the scheduler for immediate engagement). After any daily content delivery (Day 1 or Day 2+), the system waits 30 seconds before sending confirmation buttons asking if the user has read the content. This timing ensures users have adequate time to engage with content before being prompted for confirmation. Day 1 content always bypasses timezone/interval scheduling to provide immediate value upon user activation, while Day 2+ content follows the bot's configured timezone or interval schedule.
- **Asynchronous START Command Processing**: The START command handler uses background threading to prevent webhook timeouts. The welcome message is sent immediately and the webhook responds within seconds, while Day 1 content delivery (10s delay + content + 30s confirmation delay) runs in a background thread with proper Flask app context management. This ensures Telegram/WhatsApp webhooks don't timeout during the 40+ second content delivery process, allowing reliable message delivery and confirmation button display.
