    ...
    try:
        ...
        if existing_user and existing_user.status in ['active', 'inactive']:
            ...
            def deliver_day1_content_async():
                time.sleep(10)  # 10 second delay before Day 1 content (outside app context)
                ...
                try:
                    with app.app_context():  # Open context only for DB operations
...
def edit_bot(bot_id):
    ...
    if request.method == 'GET':
        ...
        form.whatsapp_connection_type.data = bot.whatsapp_connection_type if hasattr(bot, 'whatsapp_connection_type') and bot.whatsapp_connection_type else 'meta'
        ...
        form.waha_base_url.data = bot.waha_base_url if hasattr(bot, 'waha_base_url') else None
        form.waha_api_key.data = bot.waha_api_key if hasattr(bot, 'waha_api_key') else None
        form.waha_session.data = bot.waha_session if hasattr(bot, 'waha_session') and bot.waha_session else 'default'
    ...
    if form.validate_on_submit():
        try:
            ...
            bot.description = form.description.data
            bot.platforms = form.platforms.data or []
            bot.whatsapp_connection_type = form.whatsapp_connection_type.data or 'meta'
            bot.whatsapp_access_token = form.whatsapp_access_token.data if 'whatsapp' in (form.platforms.data or []) else None
            bot.whatsapp_phone_number_id = form.whatsapp_phone_number_id.data if 'whatsapp' in (form.platforms.data or []) else None
            bot.whatsapp_webhook_url = form.whatsapp_webhook_url.data if 'whatsapp' in (form.platforms.data or []) else None
            bot.whatsapp_verify_token = form.whatsapp_verify_token.data or 'CVGlobal_WhatsApp_Verify_2024'
            bot.waha_base_url = form.waha_base_url.data if 'whatsapp' in (form.platforms.data or []) else None
            bot.waha_api_key = form.waha_api_key.data if 'whatsapp' in (form.platforms.data or []) else None
            bot.waha_session = form.waha_session.data if 'whatsapp' in (form.platforms.data or []) and form.waha_session.data else 'default'
            bot.telegram_bot_token = form.telegram_bot_token.data if 'telegram' in (form.platforms.data or []) else None
